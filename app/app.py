...
def index():
    if request.method == "POST":
        ...
        author = request.form["author"]
        requests.post(
            #"http://52.23.194.245:5001/api/quotes", json={"quote": quote, "author": author}
            "http://ecs-test-735898911.us-east-1.elb.amazonaws.com/api/quotes", json={"quote": quote, "author": author}
        )
        # requests.post(
        ...
        quotes = requests.get("http://ecs-test-735898911.us-east-1.elb.amazonaws.com/api/quotes").json()
        return render_template("index.html", quotes=quotes)
        # quotes = requests.get(
        #     "http://bonni-test-env.eba-h7hph2uq.us-west-2.elasticbeanstalk.com/api/quotes"
        # ).json()
